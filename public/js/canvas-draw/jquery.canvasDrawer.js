(function ($) {
    ...
    let init = function (index, input, options) {

        let points, activePoint, settings;
        let $reset, $canvas, ctx, image;
        let draw, mousedown, stopdrag, move, moveall, resize, reset, rightclick, record;
        let dragpoint;
        let startpoint = false;

        settings = $.extend({
            imageUrl: $(this).attr('image-src')
        }, options);

        let v = $(input).val().replace(/[^0-9\,]/ig, '');
        if (v.length) {
        ...
        moveall = function (e) {
            ...
            let sdvpoint = { x: Math.round(e.offsetX), y: Math.round(e.offsetY) };
        ...
        draw = function () {
            ...
            for (let i = 0; i < points.length; i += 2) {
                ctx.fillRect(points[i] - halfSquareSize, points[i + 1] - halfSquareSize, squareSize, squareSize);
        ...
        let getCenter = function () {
            ...
            let twicearea = 0,
                ...
                nptc = ptc.length,
                p1, p2, f;
            for (let i = 0, j = nptc - 1; i < nptc; j = i++) {
                p1 = ptc[i];
                p2 = ptc[j];
    ...
    let dotLineLength = function (x, y, x0, y0, x1, y1, o) {
        ...
        if (o && !(o = function (x, y, x0, y0, x1, y1) {
            ...
            let left, tg = -1 / ((y1 - y0) / (x1 - x0));
            return {
                x: left = (x1 * (x * tg - y + y0) + x0 * (x * -tg + y - y1)) / (tg * (x1 - x0) + y0 - y1),
                y: tg * left - tg * x + y
            };
        }(x, y, x0, y0, x1, y1), o.x >= Math.min(x0, x1) && o.x <= Math.max(x0, x1) && o.y >= Math.min(y0, y1) && o.y <= Math.max(y0, y1))) {
            let l1 = lineLength(x, y, x0, y0), l2 = lineLength(x, y, x1, y1);
            return l1 > l2 ? l2 : l1;
        }
        else {
